#include "StdAfx.h"
#include "loop_completion.h"

LoopCompletion::LoopCompletion(VolumeMesh *_mesh, OvmVeH _sv, OvmVeH _ev, 
...
LoopCompletion::~LoopCompletion(void)
{

}

void LoopCompletion::shortest_path (std::deque<OvmHaEgH> &path)
{
...
LoopCompletionNode * LoopCompletion::create_first_node ()
{
	auto n = new LoopCompletionNode (start_vh);
	calc_F_value (NULL, n);
	return n;
}

void LoopCompletion::shortest_path (std::vector<OvmHaEgH> &path)
{
...
void LoopCompletion::get_next_node_candidates (LoopCompletionNode *current_node, 
	std::vector<LoopCompletionNode*> &next_node_candidates)
{
	for (auto voh_it = mesh->voh_iter (current_node->vh); voh_it; ++voh_it)
	{
		...
		auto new_node = new LoopCompletionNode (to_vh, current_node->vh);
		next_node_candidates.push_back (new_node);
	}
}

double LoopCompletion::calc_F_value (LoopCompletionNode *parent_node, LoopCompletionNode *current_node)
{
	double this_G = 1.0f;
...
int LoopCompletion::calc_H_value (OvmVeH vh1, OvmVeH vh2)
{
...
double LoopCompletion::calc_penalty (OvmEgH eh1, OvmVeH vh, OvmEgH eh2)
{
	double penalty = 0.0f;
